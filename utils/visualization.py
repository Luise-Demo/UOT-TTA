...
def draw_tsne(feature_extractor, classifier, cluster_head,
    ...
    # get features
    prototypes = cluster_head.fc.weight.data
    ...
    elif dataset == 'eeg_dataset':  # 👈 新增 EEG 数据集
        # 直接复用 office 可视化逻辑，完全适用于 EEG
        tensorboar_office(s_label_set, t_label_set, s_gt, t_gt, Y, prototype_Y, writer, mode)
    else:
...
def get_embedding(feature_extractor, classifier, s_dl, t_dl):
    with TrainingModeManager([feature_extractor], train=False) as mgr, \
        ...
        torch.no_grad():
        for i, (im, label, idx)  in enumerate(tqdm(s_dl, desc='get source feature')):
            s_gt = label.cuda()
            im = im.cuda()
    ...
    with TrainingModeManager([feature_extractor], train=False) as mgr, \
            ...
            torch.no_grad():
        for i, (im, label, idx) in enumerate(tqdm(t_dl, desc='get target feature')):
            t_gt = label.cuda()
            im = im.cuda()
